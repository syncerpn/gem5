...
class Executor():
    def __init__(self, gem5_bin, sim_sys, num_cpus, workload_list, workload_args_list, protocol, mesh_row, mesh_col, n_port_per_node, out_dir):
        ...
        if self.protocol == "moesi":
            self.num_l2caches = (self.mesh_row * self.mesh_col - self.num_cpus) // 2
            self.num_dirs = self.num_l2caches
            assert self.mesh_row * self.mesh_col == self.num_cpus + self.num_l2caches + self.num_dirs, f"[ERRO] cannot fill this mesh {self.num_cpus} CPUs + {self.num_l2caches} L2 + {self.num_dirs} Dirs"
            print(f"[INFO] protocol: {self.protocol} - filling mesh with {self.num_l2caches} L2 Caches and {self.num_dirs} Directories")
    ...
    def estimate_using_last_trace(self, new_config):
        ...
        print(f'[INFO] estimated cost {data_transfer_cost:.6f} - took: {time.time() - st:.6f} (s)')
