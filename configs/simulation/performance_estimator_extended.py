...
parser.add_argument('--split_rate', type=int, default=5, help='use n over 10 folds for training')
parser.add_argument('--dataset_file', default='/home/nghiant/git/gem5/dataset_2x4_2_all_extended.txt', help='dataset')
parser.add_argument('--num_output', type=int, default=5, help='number of factors to predict')
#output
parser.add_argument('--cv_dir', default='pe_model_2x4_2_extended/test_5/', help='checkpoint directory (models and logs are saved here)')
...
def valid(epoch):
    ...
    for i in range(args.num_output):
        log_str = '[ INFO ] VALID PARAM %2d: DIFF_MEAN: %.3f | DIFF_STD: %.3f' % (i, diff_mean[i], diff_std[i])
        print(log_str)
    ...
    torch.save(state, args.cv_dir + '/ckpt_E_%d_D_' % (epoch) + '_'.join(['%.6f' % diff_mean[i] for i in range(args.num_output)]) + '.t7')
...
def test():
    ...
    # diff = torch.abs(y - yv)
    diff = torch.mean(torch.abs(y - yv) / yv, dim=1)
...
len_x = dataset.shape[1] - args.num_output
n_datapoint = dataset.shape[0]

trainset = dataset[0:n_datapoint*args.split_rate//10, :]
Xt = trainset[:, 0:len_x]
Yt = trainset[:, len_x:len_x+args.num_output]

validset = dataset[n_datapoint*args.split_rate//10: , :]
Xv = validset[:, 0:len_x]
Yv = validset[:, len_x:len_x+args.num_output]

n_sample = Xt.shape[0]

## model
# create and load core model
core = performancer_estimator(len_x, len_y=args.num_output)
